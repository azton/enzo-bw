...
int grid::DualRadStreamTestInitializeGrid(int NumChemicals, 
...
//     fprintf(stdout,"Entering grid::RadHydroStreamTestInitializeGrid routine\n");
  ...
  int RhoNum, TENum, IENum, V0Num, V1Num, V2Num, UVNum, XrNum, DeNum, 
  ...
  if (NewData == TRUE) {
	...
	printf("   internal energy = %g\n",IEConstant);
      ...
      fprintf(stdout,"   UV radiation = %g\n",UVConstant);
      fprintf(stdout," Xray radiation = %g\n",XrConstant);
	...
	fprintf(stdout,"         nHeIII = %g\n",BaryonField[HeIIINum][1]);
      ...
      fprintf(stdout,"\n  Streaming radiation information:\n");
      fprintf(stdout,"       RadStreamDim = %"ISYM"\n",RadStreamDim);
      fprintf(stdout,"       RadStreamDir = %"ISYM"\n",RadStreamDir);
      fprintf(stdout,"      RadStreamType = %"ISYM"\n",RadStreamType);
    }

    // adjust Radiation Energy Density BC at streaming input edge
    int idx;
    int RadNum = (RadStreamType == 1) ? UVNum : XrNum;
    if (RadStreamDim == 0) {
      if (RadStreamDir == 0) {
