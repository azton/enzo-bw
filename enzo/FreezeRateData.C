...
int FreezeRateData(FLOAT Time, HierarchyEntry &TopGrid)
{
  ...
  // get current units
  float dUnit=1, lUnit=1, TUnit=1, tUnit=1, vUnit=1, mUnit=1;
  ...
  if (foundgrid == 0) {
    fprintf(stderr,"FreezeRateData could not find a level 0 grid!");
    return FAIL;
  }
  ...
  if (DualEnergyFormalism) {  // get internal energy
    ...
    if (eint_ == NULL){
      fprintf(stderr,"Could not access internal energy field!");
      return FAIL;
    }
    ...
    if (etot_ == NULL){
      fprintf(stderr,"Could not access total energy field!");
      return FAIL;
    }
  ...
  if (rho_ == NULL){
    fprintf(stderr,"Could not access Density field!");
    return FAIL;
  }
  float rho = rho_[idx];
  float *HI_ = ThisGrid->GridData->AccessHIDensity();
  if (HI_ == NULL){
    fprintf(stderr,"Could not access HIDensity field!");
    return FAIL;
  }
  float HI = HI_[idx];
