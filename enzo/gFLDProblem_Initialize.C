...
//   rank = 3;
  ...
  newt_INconst       = 1.0e-3;    // inexact Newton forcing constant
  newt_tol           = 1.0e-3;    // default nonlinear tolerance
  ...
  if (dtgrowth < 1.0 || dtgrowth > 10.0) {
    fprintf(stderr,"gFLDProblem Initialize: illegal RadHydroDtGrowth = %g\n",dtgrowth);
...
//   Eta = new float[ArrDims[0]*ArrDims[1]*ArrDims[2]];
  ...
  if (MyProcessorNumber == ROOT_PROCESSOR) {
    ...
    if ((outfptr = fopen(outfilename, "a")) == NULL) {
      ...
      fprintf(outfptr, "RadHydroDtGrowth = %"FSYM"\n", dtgrowth);
