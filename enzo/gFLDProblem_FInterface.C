...
extern "C" void FORTRAN_NAME(gfldproblem_matrixentries_3d)(
   ...
   float *adjvec, int *LimType, float *dt, 
...
extern "C" void FORTRAN_NAME(gfldproblem_diffrhs_3d)(
   ...
   int *LimType, FLOAT *a, float *aUnits, float *LenUnits, float *ErUnits, 
   float *dx, float *dy, float *dz, int *Nx, int *Ny, int *Nz, 
   int *NGxl, int *NGxr, int *NGyl, int *NGyr, int *NGzl, 
   int *NGzr, int *xlface, int *xrface, int *ylface, int *yrface, 
   int *zlface, int *zrface, int *Model, int *ier);
  
extern "C" void FORTRAN_NAME(gfldproblem_diffrhs_2d)(
   float *drhs, float *Er, float *Er0, float *Temp, float *kappaE, 
   int *LimType, FLOAT *a, float *aUnits, float *LenUnits, float *ErUnits, 
   float *dx, float *dy, int *Nx, int *Ny, int *NGxl, int *NGxr, int *NGyl, 
   int *NGyr, int *xlface, int *xrface, int *ylface, int *yrface, 
   int *Model, int *ier);
  
extern "C" void FORTRAN_NAME(gfldproblem_diffrhs_1d)(
   float *drhs, float *Er, float *Er0, float *Temp, float *kappaE, 
   int *LimType, FLOAT *a, float *aUnits, float *LenUnits, float *ErUnits, 
   float *dx, int *Nx, int *NGxl, int *NGxr, int *xlface, int *xrface, 
   int *Model, int *ier);
			       ...
			       float *sigA, float *sigS, float *adjvec) 
{
  ...
  if (rank == 3) {
    FORTRAN_NAME(gfldproblem_matrixentries_3d)
      (matentries, Er, Er0, Temperature, sigA, adjvec, &LimType, 
			 ...
			 float *Temperature, float *sigA, float *sigS)
{
  ...
  if (rank == 3) {
    FORTRAN_NAME(gfldproblem_diffrhs_3d)
      (drhs, Er, Er0, Temperature, sigA, &LimType, &a, &aUnits, 
       &LenUnits, &ErUnits, &dx[0], &dx[1], &dx[2], &LocDims[0], &LocDims[1], 
       &LocDims[2], 
       &GhDims[0][0], &GhDims[0][1], &GhDims[1][0], &GhDims[1][1], 
       &GhDims[2][0], &GhDims[2][1], &xlface, &xrface, &ylface, &yrface, 
       &zlface, &zrface, &Model, &ier);
    return(ier);
  }
  else if (rank == 2) {
    FORTRAN_NAME(gfldproblem_diffrhs_2d)
      (drhs, Er, Er0, Temperature, sigA, &LimType, &a, &aUnits, 
       ...
       &GhDims[0][0], &GhDims[0][1], &GhDims[1][0], &GhDims[1][1], &xlface, 
       &xrface, &ylface, &yrface, &Model, &ier);
    return(ier);
  }
  else {
