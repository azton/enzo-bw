...
int grid::DualCosmoIonizationInitializeGrid(int NumChemicals,
					    float VxConstant, 
					    float VyConstant, 
					    float VzConstant, 
					    float IEConstant, 
...
//     printf("Entering grid::CosmoIonizationInitializeGrid routine\n");
  ...
  int RhoNum, TENum, GENum, V0Num, V1Num, V2Num, XrNum, UVNum, 
    DeNum, HINum, HIINum, HeINum, HeIINum, HeIIINum, kphHINum, 
    kphHeINum, kphHeIINum, gammaNum, kdissH2INum;
  ...
  if (NewData == TRUE) {
    ...
    for (i=0; i<size; i++) {
      ...
      BaryonField[XrNum][i]    = XrConstant/EUnits;
      BaryonField[UVNum][i]    = UVConstant/EUnits;
