...
#pragma omp parallel private(field, dim, i, j, k, i1, j1, k1, Dims, ProjectedDims, TotalRefinement, dArea, size) shared(ProjectedFluxes, RefinementFactors, MyProcessorNumber) default(none)
#else
#pragma omp parallel private(field, dim, i, j, k, i1, j1, k1, Dims, ProjectedDims, TotalRefinement, dArea, size) shared(NumberOfBaryonFields, GridRank, BoundaryFluxes, ProjectedFluxes, RefinementFactors, GridDimension, MyProcessorNumber, ProcessorNumber) default(none)
#endif
  {
...
#pragma omp for schedule(dynamic)
    for (field = 0; field < NumberOfBaryonFields; field++) {
	  ...
	  if (i != dim) {
        ...
        dArea = 1.0/float(TotalRefinement);
...
//PRAG end of parallel region


  // Communication: send and receive fluxes
