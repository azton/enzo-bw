			     ...
			     int direction, int TransposeOnCompletion)
{
  ...
  for (i = 0, x = 0; i < NumberOfProcessors; i++) {
    ...
    strip0[i].RegionDim[0] = nint(x/(2.0*DomainCellSize[0]))*2 - strip0[i].StartIndex[0];
  ...
  for (i = 0, x = 0; i < NumberOfProcessors; i++) {
    ...
    x += (DomainRightEdge[LastIndex] - DomainLeftEdge[LastIndex])/float(NumberOfProcessors);

    strip1[i].RegionDim[LastIndex] = nint(x/DomainCellSize[LastIndex]) - strip1[i].StartIndex[LastIndex];
...
//      fprintf(stderr, "FFT(%"ISYM"): Transpose on Completion\n", MyProcessorNumber);
  ...
  if (direction == FFT_INVERSE) {
    ...
    /* Transpose from original layout to strip0/1 (if necessary). */
