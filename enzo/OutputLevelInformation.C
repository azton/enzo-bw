			   ...
			   LevelHierarchyEntry *LevelArray[])
{
  ...
  for (level = 0; level <= MaximumRefinementLevel; level++) {
    ...
    if (level <= MaximumRefinementLevel) {
      ...
      // jbPerf count-particles-local-<level>

      sprintf (jb_counter_name,"count-particles-local-%d",level);
      value = LocalParticles[level];
      jbPerf.assign(jb_counter_name, value);
    }

    // Accumulate jbPerf counter values for "count-*"
  ...
  jbPerf.assign("count-particles", jb_particles);
