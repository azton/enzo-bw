...
*/
  ...
  if (NumberOfProcessors > 1) {
    ...
    if (FirstTimeCalled) {
      ...
      //  fprintf(stderr, "Size of ParticleMoveList %d\n", Count);
...
//    delete [] SendListDisplacements;
	...
	if (TransferSize > 0 && FromProcessor != ToProcessor) {
	  ...
	  Xsize = TransferSize;
          ...
          // fprintf(stderr, "sizeof SendCount %d\n", sizeof(SendCount));
          // fprintf(stderr, "sizeof MPI_Arg %d\n", sizeof(MPI_Arg));
...
//          SendCount = TransferSize*sizeof(float_int);
	    ...
	    if( SendCount > 0 ) {
	    // fprintf(stderr, "P%d Sending %d bytes to %d [%d x %d]\n", Here, SendCount, Dest, Usize, Xsize); 
...
//          RecvCount = TransferSize*sizeof(float_int);
	    ...
	    // fprintf(stderr, "P%d Post receive %d bytes from %d [%d x %d]\n", Here, RecvCount, Source, Usize, Xsize);
	    ...
	    stat = MPI_Get_count(&Status, DataTypeByte, &RecvCount);
	      assert( stat == MPI_SUCCESS );
	    // fprintf(stderr, "P%d Received %d bytes from %d [%d]\n", Here, RecvCount, Source, Usize);
	  }
