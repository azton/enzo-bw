...
/  modified3:  Robert Harkness, April 2010
...
#define io_type float32
  ...
  ThisBlock = (GridID - 1)/BlockSize;

  // if GridID is in active block then go ahead
  ...
  if (NumberOfBaryonFields > 0 && ReadData ) {
	...
	strcat(name, id);
      ...
      file_id = H5Fopen(name,  H5F_ACC_RDONLY, file_acc_template);
  ...
  if ( (GridID) == ((ActiveBlock + 1) * BlockSize) ) {
