...
/  modified2:  Robert Harkness, March 2010
/              Divide I/O tasks into blocks to prevent overloading Lustre.
/              The number of blocks should be approximately
/              M * (NumberOfProcessors/NumberOfLustreOSTs), M = 1 or 2.
/              On NICS Kraken XT5 there are 336 OSTs.
/
/  PURPOSE:
/
************************************************************************/
...
#define io_type float32
  ...
  int NumberOfLustreOSTs = 1000;
  int NumberOfBlocks;
  int BlockSize;
  int ThisBlock;

  NumberOfBlocks = (NumberOfProcessors / NumberOfLustreOSTs);
  if (NumberOfProcessors % NumberOfLustreOSTs > 0) NumberOfBlocks++;
  BlockSize = NumberOfLustreOSTs;

  ThisBlock = (grid_id - 1)/BlockSize;

  /* Open HDF file for writing. */

  // if grid_id is in active block then go ahead
  // else wait until the block is done
  // grid_id starts at 1

  if ( ThisBlock == ActiveBlock ) {
  ...
  if (NumberOfBaryonFields > 0) {
    ...
    for (field = 0; field < NumberOfBaryonFields; field++) {
      ...
      // DumpBaryonField[MAX_NUMBER_OF_FIELD_TYPES] uses the baryon typedef.
      // If you add more fields to ENZO, be sure to increase MAX_NUMBER_OF_FIELD_TYPES
      // We cannot use MAX_NUMBER_OF_BARYON_FIELDS for this since that can be smaller
      // than the number of possible fields and it is necessary to minimize the value
      // of MAX_NUMBER_OF_BARYON_FIELDS to reduce memory requirements.

      if(DumpBaryonField[FieldType[field]]) {
...
//                                  NOTE: for 128bits this must be FILE_type_id and NOT FLOAT_type_id!
  ...
  }  // End of active task block


  if ( (grid_id) == ((ActiveBlock + 1) * BlockSize) ) {
    // Barrier synchronization for this task block
