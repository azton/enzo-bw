...
//  }
  ...
  if (NumberOfBaryonFields > 0) {
	...
	for (j = GridStartIndex[1]; j <= GridEndIndex[1]; j++)
      ...
      file_status = FileMover(DataLabel[field]);
	...
	for (j = GridStartIndex[1]; j <= GridEndIndex[1]; j++)
      ...
      file_status = FileMover("Temperature");
        assert( file_status == 0 );
	...
	for (j = StartIndex[1]; j <= EndIndex[1]; j++)
      ...
      file_status = FileMover("Dark_Matter_Density");
        assert( file_status == 0 );
...
if ( 0 == 1 )
{
  ...
  if (NumberOfParticles > 0) {
	...
	temp_pointer = (float64*) ParticlePosition[dim];
      ...
      file_status = FileMover(ParticlePositionLabel[dim]);
        assert( file_status == 0 );
    }
	...
	temp[i] = float32(ParticleVelocity[dim][i]);
      ...
      file_status = FileMover(ParticleVelocityLabel[dim]);
        assert( file_status == 0 );
    }
    ...
    file_status = FileMover("particle_mass");
      assert( file_status == 0 );
    ...
    file_status = FileMover("particle_index");
      assert( file_status == 0 );
	...
	temp[i] = float32(ParticleAttribute[j][i]);
      ...
      file_status = FileMover(ParticleAttributeLabel[j]);
        assert( file_status == 0 );
