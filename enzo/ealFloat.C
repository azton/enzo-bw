...
void ealFloat::ReduceSum(){
  ...
  if( mpi_err != MPI_SUCCESS ){
    fprintf(stderr, "ealFloat::ReduceSum, mpi_err = %d, exiting.\n", mpi_err);
    my_exit(EXIT_FAILURE);
  }
...
void ealFloat::ReduceMin(){
  ...
  if( mpi_err != MPI_SUCCESS ){
    fprintf(stderr, "ealFloat::ReduceMax, mpi_err = %d, exiting.\n", mpi_err);
    my_exit(EXIT_FAILURE);
  }
...
void ealFloat::Bcast(int FromProcessor){
  ...
  if( mpi_err != MPI_SUCCESS ){
    fprintf(stderr, "ealFloat::Bcast, mpi_err = %d, exiting.\n", mpi_err);
    my_exit(EXIT_FAILURE);
  }
#endif
...
float &ealFloat::operator[](int subscript){
  ...
  if(subscript < 0 || subscript >=Size){
    fprintf(stderr, "ealFloat: subscript %d out of range.\n", subscript);
    my_exit(EXIT_FAILURE);
  }
...
const ealFloat &ealFloat::operator=(const ealFloat &right){
  ...
  if(Size != right.Size && right.Size != 1){
    fprintf(stderr, "ealFloat::assignment Size %d != right.Size %d.\n", Size, right.Size);
    my_exit(EXIT_FAILURE);
  }
...
&ealFloat::operator+=(const ealFloat &right){
  ...
  if(Size != right.Size && right.Size != 1){
    fprintf(stderr, "ealFloat::Math Error Size %d != right.Size %d.\n", Size, right.Size);
    my_exit(EXIT_FAILURE);
  }
