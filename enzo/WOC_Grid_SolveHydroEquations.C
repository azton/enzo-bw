...
/  date:       January, 2009 - Added Shock Mach Handling
/
/  PURPOSE:
/
/  RETURNS:
			      ...
			      fluxes *SubgridFluxes[], int level)
{
  ...
  if (NumberOfBaryonFields > 0) {
    ...
    // use different color fields for RadiationHydrodynamics problems
    ...
    //   second, the color field advection if using RadiationHydrodynamics
    if (MultiSpecies > 0 && RadiationHydrodynamics == 1) {
      NumberOfColours = 7 + 3*(MultiSpecies-1);

      /* Generate an array of field numbers corresponding to the colour fields.
	 First set the radiation field. */
      if ((ColourNum =
	...
	fprintf(stderr, "Could not find RadiationFreq0.\n");
      ...
      /* Continue with the ElectronDensity and continue in order. */
      if ((ColourNum =
           FindField(ElectronDensity, FieldType, NumberOfBaryonFields)) < 0) {
        fprintf(stderr, "Could not find ElectronDensity.\n");
        return FAIL;
