...
#pragma omp parallel private(field, i, j, k, index) shared(RadiativeCooling, Start, End, DensNum, Offset, FieldDensityType) default(none)
#else
...
#pragma omp for schedule(dynamic)
	  for (field = 0; field < NumberOfBaryonFields; field++) {
	    ...
	    if (FieldTypeIsDensity(FieldType[field]) == FALSE &&
		 (RadiativeCooling == 0 || (FieldType[field] != TotalEnergy && FieldType[field] != InternalEnergy))) {
...
#ifdef INTEL_OMP_SYNTAX
#pragma omp parallel private(field, i, j, k, index) shared(Start, End, Offset, DensNum, FieldDensityType) default(none)
#else
#pragma omp parallel private(field, i, j, k, index) shared(NumberOfBaryonFields, FieldType, Start, End, GridDimension, BaryonField, Offset, DensNum, FieldDensityType) default(none)
#endif
  {
#pragma omp for schedule(dynamic) 
	for (field = 0; field < NumberOfBaryonFields; field++) {

	  if (FieldTypeIsDensity(FieldType[field]) == TRUE && FieldType[field] != Density) {
	...
	for (field = 0; field < NumberOfBaryonFields; field++) {
	  ...
	  for (k = Start[2]; k <= End[2]; k++) {
		...
		FluxIndex  = ((k - Start[2])*Dim[1] + (j - Start[1]))*Dim[0] + (i - Start[0]);
...
#ifdef INTEL_OMP_SYNTAX
#pragma omp parallel private(field, i, j, k, index) shared(RadiativeCooling, Start, End, Offset, DensNum, FieldDensityType) default(none)
#else
...
#ifdef INTEL_OMP_SYNTAX
#pragma omp parallel private(i, j, k, i1, i2) shared(Start, End, Offset, tiny_number, GENum, TENum, Vel1Num, Vel2Num, Vel3Num) default(none)
#else
...
#ifdef INTEL_OMP_SYNTAX
#pragma omp parallel private(field, i, j, k, index) shared(Start, End, DensNum, Offset, FieldDensityType) default(none)
#else
#pragma omp parallel private(field, i, j, k, index) shared(NumberOfBaryonFields, FieldType, Start, End, GridDimension, BaryonField, DensNum, Offset, FieldDensityType) default(none)
#endif
  {
#pragma omp for schedule(dynamic) 
	for (field = 0; field < NumberOfBaryonFields; field++) {
