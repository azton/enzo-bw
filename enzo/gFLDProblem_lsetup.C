...
//   if (debug)  printf("Entering gFLDProblem::lsetup routine\n");
  ...
  // in case we are using the AnalyticChem solver with Model 4 (isothermal), we need to put something in the J_{ec,ec} block to avoid numerical problems (even though it is decoupled from the other equations).
  if ((AnalyticChem == 1) && (Model == 4))   (L[1])->addconst_component(1,1.0);
